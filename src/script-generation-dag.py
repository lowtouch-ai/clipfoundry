...
def request_user_approval(**kwargs):
    ...
    # 2. Build Unified Markdown Report
...
"""
    ...
    if "email_data" in dag_run_conf:
        ...
        if user_email and "@" in user_email:
            try:
                # Wrap Markdown in a pre-styled block for email readability
                email_body = f"""
                <div style="font-family: sans-serif;">
                    <h3>Script Generation Complete</h3>
                    <pre style="background: #f4f4f4; padding: 15px; border-radius: 5px; font-family: sans-serif; white-space: pre-wrap;">
{report_content}
                    </pre>
                </div>
                """
    ...
    # B. API / Agent Mode
        ...
        return {
            "status": "success",
            "markdown_output": report_content, 
            "raw_data": data 
        }
